...
for i in range(1):
  ...
  # mat_mul 결과 확인하기
  temp_matmul_out = sess.run(temp_matmul, feed_dict={X: x_data, Y: y_data})

  # y 값 확인하기
  temp_y_out = sess.run(temp_y, feed_dict={X: x_data, Y: y_data})
  y_out = sess.run(y, feed_dict={X: x_data, Y: y_data})
  guess_out = sess.run(guess, feed_dict={X: x_data, Y: y_data})
